...
export async function getRounds(leagueId = DEFAULT_LEAGUE_ID, { includePlayoffs = false } = {}) {
  const pool = getPool();
  const playoffClause = includePlayoffs ? '' : 'AND is_playoff = FALSE';
  const { rows } = await pool.query(
    `SELECT id, round_number, name, is_playoff, starts_at, ends_at
     FROM rounds
     WHERE league_id = $1 ${playoffClause}
...
export async function getStandings(leagueId = DEFAULT_LEAGUE_ID) {
  ...
  const { rows: matchups } = await pool.query(
    `SELECT m.home_team_id, m.away_team_id, m.home_points, m.away_points, m.result, m.status
     FROM h2h_matchups m
     JOIN rounds r ON r.id = m.round_id
     WHERE m.league_id = $1
...
export async function getLeagueSummary(leagueId = DEFAULT_LEAGUE_ID) {
  ...
  const playoffStatus = hasSchedule
    ? await import('./playoffLeague').then((m) => m.getPlayoffStatus(leagueId))
    : null;
