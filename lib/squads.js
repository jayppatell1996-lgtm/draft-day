...
import { calculateBudgetRemaining, validateSwap } from './squadValidation';
...
export async function getSquadByTeamId(fantasyTeamId, leagueId = DEFAULT_LEAGUE_ID) {
  ...
  const { rows } = await pool.query(
    `SELECT id, league_id, fantasy_team_id, captain_player_id, vice_captain_player_id,
            budget_remaining, free_trades_banked, created_at, updated_at
     FROM squads
...
export async function assignPlayerToSlot({
  ...
  const player = await getPlayerWithPrice(playerId, leagueId);
  ...
  await pool.query(
    ...
    [playerId, slotId, squadData.squad.id]
  );
...
export async function setCaptain({
  ...
  const pool = getPool();
  const column = role === 'vice' ? 'vice_captain_player_id' : 'captain_player_id';
  await pool.query(
    `UPDATE squads SET ${column} = $1, updated_at = NOW() WHERE id = $2`,
