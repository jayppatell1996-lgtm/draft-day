...
import { advancePlayoffBracket } from './playoffLeague';
...
export async function getRoundByNumber(roundNumber, leagueId = DEFAULT_LEAGUE_ID) {
  ...
  const { rows } = await pool.query(
    `SELECT id, round_number, name, is_playoff, playoff_stage FROM rounds
...
export async function recalculateRoundScores(roundNumber, leagueId = DEFAULT_LEAGUE_ID) {
  ...
  try {
    ...
    let playoffAdvance = null;
    if (round.is_playoff) {
      playoffAdvance = await advancePlayoffBracket(leagueId);
    }

    return {
      roundNumber,
      roundId: round.id,
      isPlayoff: round.is_playoff,
